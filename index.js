...
// ── Keywords especiales base (hardcodeadas) ──────────────────────────
...
const SECTORES = {
  'Sector Base': [
    'REPORTES BOX DELIVERY','REPORTES BOX DELIVERY ','Hola','Hola '
  ],
  'Sector PTB': [
...
function loadKeywords() {
  ...
  // frasesDesactivadas: { 'REPORTES BOX DELIVERY': ['pendiente compra de cliente'], ... }
...
function loadSnapshot() {
  ...
  return null; // null = no hay modo enfoque activo
}
...
function getKeywordsEspeciales(grupo) {
  var base = KEYWORDS_ESPECIALES_BASE[grupo] || [];
  var din  = (KW.especiales && KW.especiales[grupo]) || [];
  return base.concat(din);
}
// Devuelve las frases del Sector Base para un grupo, filtrando las desactivadas
function getFrasesActivasSectorBase(nombreGrupo) {
  var cfg2 = null;
  var keys = Object.keys(SECTOR_BASE_CONFIG);
  for (var i = 0; i < keys.length; i++) {
    if (keys[i].trim().toLowerCase() === nombreGrupo.trim().toLowerCase()) { cfg2 = SECTOR_BASE_CONFIG[keys[i]]; break; }
  }
  if (!cfg2) return [];
...
function tieneHoraFuturaLejana(texto) {
  ...
  while ((match = regex.exec(t)) !== null) {
    var hh = parseInt(match[1],10), mm = parseInt(match[2],10), ampm = match[3];
    if (hh > 23 || mm > 59) continue;
    if (ampm === 'pm' && hh < 12) hh += 12;
    if (ampm === 'am' && hh === 12) hh = 0;
    if ((hh * 60 + mm) - horaActualMin > 15) return true;
  }
  return false;
}
...
function getSectorDeGrupo(nombreGrupo) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var grupos = SECTORES[keys[i]];
    for (var j = 0; j < grupos.length; j++) {
      if (grupos[j].trim().toLowerCase() === nombreGrupo.trim().toLowerCase()) return keys[i];
    }
  }
  return 'Sector X (otros)';
}
function esGrupoSinRemarcar(nombreGrupo) {
  if (getSectorDeGrupo(nombreGrupo) === SECTOR_COMODIN) return true;
  return GRUPOS_SIN_REMARCAR.some(function(n) { return n.trim().toLowerCase() === nombreGrupo.trim().toLowerCase(); });
}
function esGrupoGanancias(nombreGrupo) {
  return GRUPO_GANANCIAS.some(function(n) { return n.trim().toLowerCase() === nombreGrupo.trim().toLowerCase(); });
}
function procesarMensajeSectorBase(nombreGrupo, numero, texto) {
  ...
  // Usa solo frases activas (no desactivadas)
...
function extraerEntradas(texto) {
  ...
  texto.split('\n').forEach(function(linea) {
    var trimmed = linea.trim(); if (!trimmed) return;
    var mG = trimmed.match(/^menos\s+(\d{1,6}(?:\.\d{1,2})?)\s*(.*)?$/i);
    ...
    var mL = trimmed.match(/^([a-zA-ZáéíóúÁÉÍÓÚñÑ][a-zA-ZáéíóúÁÉÍÓÚñÑ\s]{0,40}?)\s+(\d{1,5}(?:\.\d{1,2})?)$/);
    ...
    var mI = trimmed.match(/^(\d{1,5}(?:\.\d{1,2})?)\s+([a-zA-ZáéíóúÁÉÍÓÚñÑ][a-zA-ZáéíóúÁÉÍÓÚñÑ\s]{0,40})$/);
    ...
    var mN = trimmed.match(/^(\d{1,5}(?:\.\d{1,2})?)$/);
...
client.on('message', async function(msg) {
  ...
  if (esGrupoGanancias(chat.name)) {
    ...
    if (texto.trim().toLowerCase()==='reset') {
      var hR=getHoraPeru().toLocaleDateString('es-PE');
      saveGanancias({fecha:hR,ganancias:0,gastos:0});
    ...
    if (entradas.length>0) {
      var ganData=loadGanancias(); var hoy=getHoraPeru().toLocaleDateString('es-PE');
      if(ganData.fecha!==hoy) ganData={fecha:hoy,ganancias:0,gastos:0};
      var rep=loadReporte(); var lA=getFechaLunesActual();
...
app.post('/enfoque', function(req,res){
  ...
  saveSnapshot({gruposActivos:[...GRUPOS_ACTIVOS],grupoEnfoque:grupoNombre});
  // Desactiva todos, activa solo el elegido
  GRUPOS_ACTIVOS=[];
  GRUPOS_CACHE.forEach(function(g){
    if(g.id===grupoId)GRUPOS_ACTIVOS.push(g.id);
  });
  saveConfig();
  res.json({ok:true,grupoEnfoque:grupoNombre});
});
...
app.get('/enfoque/estado', function(req,res){
  var snap=loadSnapshot();
  res.json(snap?{activo:true,grupoEnfoque:snap.grupoEnfoque}:{activo:false});
});
...
app.get('/', function(req, res) {
  ...
  var snap = loadSnapshot();
  var modoEnfoque = snap && snap.activo !== false;
  var grupoEnfoqueNombre = snap ? snap.grupoEnfoque : '';

  // Delay opciones
  var delayOpts='';
  for(var ms=100;ms<=1000;ms+=100) delayOpts+='<option value="'+ms+'"'+(DELAY===ms?' selected':'')+'>'+ms+' ms</option>';

  // ── Keywords dinámicas HTML (para el menú) ──
  ...
  if(!gCE.length){kwEspecialesHtml='<span style="color:#aaa;font-size:12px">Sin keywords extras por grupo</span>';}
    ...
    kwEspecialesHtml+=(kwActual.especiales[g]||[]).map(function(k){
      return '<span style="display:inline-flex;align-items:center;gap:4px;background:#fff3e0;border:1px solid #ffcc80;border-radius:16px;padding:3px 10px;font-size:12px;color:#e65100;margin:2px">'+
        k+'<button onclick="quitarKwEspecial(\''+g.replace(/'/g,"\\'")+'\',\''+k.replace(/'/g,"\\'")+'\')\" style="background:none;border:none;cursor:pointer;color:#e74c3c;font-size:14px;padding:0;line-height:1">×</button></span>';
    }).join('')+'</div>';
  });}

  // ── Frases Sector Base HTML ──
  var frasesBaseHtml='';
  Object.keys(SECTOR_BASE_CONFIG).forEach(function(grupo){
    ...
    SECTOR_BASE_CONFIG[grupo].frases.forEach(function(frase){
      ...
      frasesBaseHtml+='<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 10px;background:'+(activa?'#e8f5e9':'#f5f5f5')+';border-radius:8px;margin-bottom:4px">'+
        ...
        '<button onclick="toggleFraseBase(\''+grupo.replace(/'/g,"\\'")+'\',\''+frase.replace(/'/g,"\\'")+'\','+(activa?'true':'false')+')" '+
        'style="padding:4px 12px;border-radius:14px;border:none;background:'+(activa?'#25D366':'#ccc')+';color:white;cursor:pointer;font-size:11px">'+
        (activa?'ON ✅':'OFF ⛔')+'</button></div>';
    });
    frasesBaseHtml+='</div>';
  });

  // Select grupos para keywords especiales
  ...
  Object.keys(porSector).forEach(function(sector){
    ...
    var gruposHtml=grupos.map(function(g){
      ...
      return '<div class="grupo-item" data-nombre="'+g.name.toLowerCase()+'" data-id="'+g.id+'" data-nombrereal="'+g.name.replace(/"/g,'&quot;')+'" style="display:flex;justify-content:space-between;align-items:center;padding:10px 0 10px 16px;border-bottom:1px solid #f0f0f0;'+op+'">'+
        '<span style="font-size:13px;color:#444">'+g.name+fotoTag+tagM+tagC+tagB+cdInfo+'</span>'+
        ...
        '<button onclick="soloEste(\''+g.id+'\',\''+g.name.replace(/'/g,"\\'")+'\')" title="Solo este grupo" style="padding:5px 10px;border-radius:20px;border:none;background:#e67e22;color:white;cursor:pointer;font-size:12px">🎯</button>'+
        '</div></div>';
    }).join('');
  ...
  res.send('<!DOCTYPE html><html><head>'+
    ...
    '.kw-tag-g{display:inline-flex;align-items:center;gap:4px;background:#e8f5e9;border:1px solid #a5d6a7;border-radius:16px;padding:3px 10px;font-size:12px;color:#2e7d32;margin:3px;}'+
    '.kw-tag-e{display:inline-flex;align-items:center;gap:4px;background:#fdecea;border:1px solid #ef9a9a;border-radius:16px;padding:3px 10px;font-size:12px;color:#c62828;margin:3px;}'+
    '.kw-tag-s{display:inline-flex;align-items:center;gap:4px;background:#fff3e0;border:1px solid #ffcc80;border-radius:16px;padding:3px 10px;font-size:12px;color:#e65100;margin:2px;}'+
    '.kw-del{background:none;border:none;cursor:pointer;color:#e74c3c;font-size:14px;padding:0;line-height:1;}'+
    '.kw-input{flex:1;padding:7px 10px;border-radius:8px;border:1px solid #ddd;font-size:13px;}'+
    '.kw-btn-g{padding:7px 12px;border-radius:8px;border:none;background:#25D366;color:white;cursor:pointer;font-size:13px;}'+
    '.kw-btn-e{padding:7px 12px;border-radius:8px;border:none;background:#e74c3c;color:white;cursor:pointer;font-size:13px;}'+
    '.kw-btn-s{padding:7px 12px;border-radius:8px;border:none;background:#e67e22;color:white;cursor:pointer;font-size:13px;}'+
    '</style>'+
    ...
    '<a href="/historial" class="menu-link">📋 Historial <span style="color:#888;font-size:12px">('+HISTORIAL.length+')</span></a>'+
    '</div>'+
    ...
    '<button onclick="guardarDelay()" style="padding:8px 14px;border-radius:8px;border:none;background:#3498db;color:white;cursor:pointer;font-size:13px">Guardar</button></div>'+
    '</div>'+

    // — Keywords globales —
    '<div class="menu-section"><h4>✅ Keywords globales (activan el bot)</h4>'+
    ...
    '<button onclick="agregarKw(\'global\')" class="kw-btn-g">+</button></div></div>'+

    // — Keywords excluir —
    '<div class="menu-section"><h4>🚫 Keywords excluidas (bloquean respuesta)</h4>'+
    ...
    '<button onclick="agregarKw(\'excluir\')" class="kw-btn-e">+</button></div></div>'+
    ...
    '<button onclick="agregarKwEspecial()" class="kw-btn-s">+</button></div></div>'+
    ...
    // — Cerrar sesión —
    '<div class="menu-section">'+
    '<button class="menu-btn" style="color:#e74c3c" onclick="if(confirm(\'¿Cerrar sesión?\')){fetch(\'/cerrar-sesion\',{method:\'POST\'}).then(function(){location.reload()})}">🚪 Cerrar sesión (escanear QR nuevo)</button>'+
    '</div>'+
    ...
    // Banner modo enfoque
    (modoEnfoque?
      '<div style="padding:12px 14px;background:#fff3e0;border:2px solid #ffcc80;border-radius:10px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center">'+
      '<div><div style="font-size:12px;color:#e65100;font-weight:600">🎯 MODO ENFOQUE</div>'+
      '<div style="font-size:13px;color:#444;margin-top:2px">'+grupoEnfoqueNombre+'</div></div>'+
      '<button onclick="restaurarConfig()" style="padding:8px 14px;border-radius:20px;border:none;background:#e67e22;color:white;cursor:pointer;font-size:13px">Restaurar 🔄</button></div>'
      :'') +
    ...
    // Toast
    'function toast(msg,color){color=color||"#25D366";var t=document.createElement("div");t.style.cssText="position:fixed;top:16px;left:50%;transform:translateX(-50%);background:"+color+";color:white;padding:10px 18px;border-radius:12px;font-size:13px;box-shadow:0 4px 12px rgba(0,0,0,.2);z-index:9999;max-width:280px;text-align:center;white-space:pre-line";t.textContent=msg;document.body.appendChild(t);setTimeout(function(){t.remove()},3000);}'+
    ...
    '  if(!confirm("¿Activar modo enfoque para: "+nombre+"?"))return;'+
    '  await fetch("/enfoque",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({grupoId:id,grupoNombre:nombre})});'+
    '  toast("🎯 Modo enfoque: "+nombre,"#e67e22");'+
    ...
    '  var kw=input.value.trim();if(!kw){toast("Escribe una keyword","#e74c3c");return;}'+
    ...
    '  if(r.ok){input.value="";toast("✅ Keyword agregada");setTimeout(function(){location.reload();},600);}'+
    '  else toast("Error al agregar","#e74c3c");'+
    '}'+
    ...
    '  if(r.ok){toast("🗑 Eliminada");setTimeout(function(){location.reload();},600);}'+
    '  else toast("Error","#e74c3c");'+
    '}'+
    'async function agregarKwEspecial(){'+
    '  var g=document.getElementById("kwEspecialGrupo").value;'+
    '  var kw=document.getElementById("kwEspecialInput").value.trim();'+
    '  if(!g){toast("Selecciona un grupo","#e74c3c");return;}'+
    '  if(!kw){toast("Escribe una keyword","#e74c3c");return;}'+
    '  var r=await fetch("/keywords/especial",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accion:"agregar",grupo:g,keyword:kw})});'+
    '  if(r.ok){document.getElementById("kwEspecialInput").value="";toast("✅ Keyword especial agregada");setTimeout(function(){location.reload();},600);}'+
    '  else toast("Error","#e74c3c");'+
    '}'+
    'async function quitarKwEspecial(g,kw){'+
    '  var r=await fetch("/keywords/especial",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accion:"quitar",grupo:g,keyword:kw})});'+
    '  if(r.ok){toast("🗑 Eliminada");setTimeout(function(){location.reload();},600);}'+
    '  else toast("Error","#e74c3c");'+
    '}'+
    ...
    '  var accion=estaActiva?"desactivar":"activar";'+
    '  var r=await fetch("/keywords/frase-base",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({grupo:grupo,frase:frase,accion:accion})});'+
    '  if(r.ok){toast(estaActiva?"⛔ Frase desactivada":"✅ Frase activada");setTimeout(function(){location.reload();},600);}'+
    '  else toast("Error","#e74c3c");'+
    '}'+
